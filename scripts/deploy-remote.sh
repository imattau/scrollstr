...
APP_NAME="scrollstr"
SERVICE_NAME="scrollstr"
INSTALL_DIR="/var/www/scrollstr"
...
SSH_TARGET=""
SSH_LOGIN_USER=""
SSH_CONTROL_PATH="${TMPDIR:-/tmp}/scrollstr-ssh-control"
...
render_header() {
	...
	printf '\n'
	hr
	panel_line "$(status_info "scrollstr deploy installer")"
	hr
...
Environment overrides:
  SCROLLSTR_SSH_TARGET, SCROLLSTR_PORT, SCROLLSTR_INSTALL_DIR,
  SCROLLSTR_SERVICE_USER, SCROLLSTR_SERVICE_GROUP, SCROLLSTR_PROXY,
  SCROLLSTR_DOMAIN, SCROLLSTR_CADDY_EMAIL, SCROLLSTR_SSH_PORT,
  SCROLLSTR_DRY_RUN, SCROLLSTR_SKIP_BUILD
EOF
}
...
printf '\n'
hr
panel_line "scrollstr remote installer"
hr
panel_line "install  : ${INSTALL_DIR}"
panel_line "service  : ${SERVICE_NAME}"
panel_line "port     : ${PORT}"
...
detect_caddy_snippet_dir() {
	...
	local dir
	if [[ -n "${SCROLLSTR_CADDY_SNIPPET_DIR:-}" ]]; then
		printf '%s' "$SCROLLSTR_CADDY_SNIPPET_DIR"
		return 0
	fi
	if [[ -f "$main_file" ]]; then
...
${import_line}"
		...
		if grep -qE '^[[:space:]]*import[[:space:]].*(/etc/caddy/)?(conf\.d|Caddyfile\.d)/\*\.caddy' "$main_file"; then
			...
			log "existing Caddyfile is managed by scrollstr; adding snippet import"
...
detect_nginx_style() {
	...
	if [[ -n "${SCROLLSTR_NGINX_STYLE:-}" ]]; then
		printf '%s' "$SCROLLSTR_NGINX_STYLE"
		return 0
	fi
...
if [[ -z "$SSH_TARGET" ]]; then
	SSH_TARGET="${SCROLLSTR_SSH_TARGET:-}"
fi
if [[ -z "$SSH_TARGET" ]]; then
	usage
...
if [[ "$PORT" == "3000" && -n "${SCROLLSTR_PORT:-}" ]]; then
	PORT="$SCROLLSTR_PORT"
fi
if [[ "$INSTALL_DIR" == "/var/www/scrollstr" && -n "${SCROLLSTR_INSTALL_DIR:-}" ]]; then
	INSTALL_DIR="$SCROLLSTR_INSTALL_DIR"
fi
if [[ "$SERVICE_USER" == "www-data" && -n "${SCROLLSTR_SERVICE_USER:-}" ]]; then
	SERVICE_USER="$SCROLLSTR_SERVICE_USER"
fi
if [[ "$SERVICE_GROUP" == "www-data" && -n "${SCROLLSTR_SERVICE_GROUP:-}" ]]; then
	SERVICE_GROUP="$SCROLLSTR_SERVICE_GROUP"
fi
if [[ "$PROXY_MODE" == "auto" && -n "${SCROLLSTR_PROXY:-}" ]]; then
	PROXY_MODE="$SCROLLSTR_PROXY"
fi
if [[ -z "$DOMAIN" ]]; then
	DOMAIN="${SCROLLSTR_DOMAIN:-}"
fi
if [[ -z "$CADDY_EMAIL" ]]; then
	CADDY_EMAIL="${SCROLLSTR_CADDY_EMAIL:-}"
fi
if [[ "$SSH_PORT" == "22" && -n "${SCROLLSTR_SSH_PORT:-}" ]]; then
	SSH_PORT="$SCROLLSTR_SSH_PORT"
fi
if [[ "$DRY_RUN" == false && -n "${SCROLLSTR_DRY_RUN:-}" ]]; then
	is_true "$SCROLLSTR_DRY_RUN" && DRY_RUN=true || true
fi
if [[ "$SKIP_BUILD" == false && -n "${SCROLLSTR_SKIP_BUILD:-}" ]]; then
	is_true "$SCROLLSTR_SKIP_BUILD" && SKIP_BUILD=true || true
fi
